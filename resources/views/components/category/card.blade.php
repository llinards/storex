<div class="carousel-cell category-cell flex flex-col justify-between rounded-lg bg-white shadow-md sm:relative">
    <a {{ $attributes }} class="block overflow-hidden h-56 lg:h-64">
        <img class="h-full w-full rounded-t-lg object-cover" src="{{ $image ?? '' }}" alt="" />
        @if ($featured)
        <div class="absolute left-0 top-2 z-10 rounded-md bg-storex-red p-1" role="is-featured"
    ...
    <div class="h-56  bg-white p-5 shadow-md lg:h-64">
        <div>
            <div>
                ...
                <div class="flex flex-col justify-between pb-4">
                    ...
                    <div class="flex items-center pb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" class="me-2 flex-shrink-0" width="20" height="20"
                            fill="#353535" viewBox="0 0 256 256">
                            <path
                                d="M235.32,73.37,182.63,20.69a16,16,0,0,0-22.63,0L20.68,160a16,16,0,0,0,0,22.63l52.69,52.68a16,16,0,0,0,22.63,0L235.32,96A16,16,0,0,0,235.32,73.37ZM84.68,224,32,171.31l32-32,26.34,26.35a8,8,0,0,0,11.32-11.32L75.31,128,96,107.31l26.34,26.35a8,8,0,0,0,11.32-11.32L107.31,96,128,75.31l26.34,26.35a8,8,0,0,0,11.32-11.32L139.31,64l32-32L224,84.69Z" />
                        </svg>
                        <p class="text-small">
                            <strong>@lang('Garums'):</strong> {{ $length }} m
                        </p>
                    </div>
                    @endif
                    @if($width->isNotEmpty())
                    <div class="flex items-center pb-2">
                        ...
                        <p class="text-small">
                            <strong>@lang('Platums'):</strong> {{ $width }} m
                        </p>
                    </div>
                    @endif
                    @if($area->isNotEmpty())
                    <div class="flex items-center pb-2">
                        ...
                        <p class="text-small">
                            <strong>@lang('Augstums'):</strong> {{ $height }} m
                        </p>
                    </div>
                    @endif
                    @if($area->isNotEmpty())
    ...
    <a class="flex justify-center rounded-b-lg bg-storex-red py-2 font-bold text-white transition-all duration-200 hover:shadow-md"
        aria-label="@lang('Skatīt produktus')" {{ $attributes }}>
        {{ $link }}
    </a>
</div>