            ...
            @foreach ($product->images as $image)
                <div class="product-cell h-72 sm:h-96">
                    <a href="{{ Storage::url('products/' . $image->filename) }}" data-fancybox="gallery">
                        <img class="h-full w-full object-cover" src="{{ Storage::url('products/' . $image->filename) }}"
                             alt=""/>
                    </a>
                </div>
            @endforeach
        </div>
        <div id="product-slider" class="product product-nav hidden gap-10 sm:block lg:mt-8"
                ...
                @foreach ($product->variants as $variant)
                    <li class="flex items-center pb-2">
                        <input class="input-radio" type="radio" id="{{ Str::slug($variant->title, '_') }}"
                               name="product_variant" value="{{ $variant->title }}" {{ $loop->first ? 'checked' : '' }}
                        />
        ...
        <div class="pt-2">
            <ul class="sm:flex">
                <li
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon mr-8 sm:mr-0" width="36" height="36"
                ...
                <li
                    ...
                    <p class="pt-2 sm:text-[10px]">@lang('4 slāņu audums')</p>
                </li>
                <li
                    class="flex items-center border-b-1 border-storex-grey py-2 sm:mr-2 sm:w-14 sm:flex-col sm:border-b-0 sm:py-0 sm:text-center md:mr-4 lg:mr-6 xl:mr-8">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon mr-8 sm:mr-0" width="36" height="36"
                ...
                <li
                    ...
                    <p class="pt-2 sm:pb-2 sm:text-[10px] md:pb-0">
                        @lang('Izturība pret temperatūras izmaiņām -30°C /+50°C')
                    </p>
                </li>
                <li
                    class="flex items-center border-b-1 border-storex-grey py-2 sm:mr-2 sm:w-14 sm:flex-col sm:border-b-0 sm:py-0 sm:text-center md:mr-4 lg:mr-6 xl:mr-8">
