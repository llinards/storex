...
it('returns a page with all products', fn() => $this->get(route('category.index', 'lv'))->assertStatus(200));
...
it('returns a page of products belonging to a category', function () {
    ...
    $this->get(route('category.show', ['lv', $category->slug]))->assertStatus(200);
});
...
it('returns a detailed page of product', function () {
    ...
    $product  = Product::factory()->create(['category_id' => $category->id]);
    ProductImage::factory(1)->create(['product_id' => $product->id]);
    $this->get(route('product.show', ['lv', $category->slug, $product->slug]))->assertStatus(200);
});
